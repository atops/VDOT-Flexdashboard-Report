...
events_bucket = conf['bucket']
config_bucket = events_bucket

session = boto3.Session(profile_name=conf['profile'])
...
def get_eventlog_data(bucket, signalid, dates):
    for date_ in dates:
        ...
        df = pd.read_parquet(
            f's3://{bucket}/atspm/date={date_str}/atspm_{signalid}_{date_str}.parquet', 
            storage_options={'profile': conf['profile']})
...
def widen(s, date_, det_config=None, source='s3'): # or source = 'db'
    ...
    desired_cols = ['Timestamp','SignalID','EventCode','EventParam','date',
    ...
    missing_data = pd.DataFrame(None, columns = missing_cols, index = df.index)
    ...
    df.to_parquet(
        f's3://{events_bucket}/atspm_wide/date={date_str}/atspm_wide_{s}_{date_str}.parquet', 
        storage_options={'profile': conf['profile']})
