...
$("body").on("shown.bs.tab", "a[data-toggle='tab']", function(e) {
    ...
    Shiny.setInputValue("active_tab", $(e.target).parent()[0].baseURI.match("section-(.*)")[1]);
...
# When changing [before] dates, update others to align
observeEvent(input$before_date_range, {
    after_duration = input$after_date_range[2] - input$after_date_range[1]

    print(glue("date range updated, {input$before_date_range}, {after_duration}"))
    updateDateRangeInput(
        session,
        "after_date_range",
        start = input$before_date_range[2] + days(1), 
        end = min(input$before_date_range[2] + after_duration + days(1), today()))
    updateDateRangeInput(
        session,
        "before_date_range",
        start = min(input$before_date_range[1], input$before_date_range[2] - days(1)),
        end = max(input$before_date_range[1] + days(1), input$before_date_range[2]))
}, ignoreInit = TRUE)


# When changing [after] dates, update others to align
observeEvent(input$after_date_range, {
    before_duration = input$before_date_range[2] - input$before_date_range[1]

    updateDateRangeInput(
        session,
        "after_date_range",
        start = min(input$after_date_range[1], input$after_date_range[2] - days(1)))
    updateDateRangeInput(
        session,
        "before_date_range",
        start = min(input$before_date_range[1], input$before_date_range[2] - before_duration),
        end = input$after_date_range[1] - days(1))
}, ignoreInit = TRUE)
...
observeEvent(input$bf_y_axis, {
    tags$script("let elem=document.getElementById('section-scatter'); let lis=elem.getElementsByTagName('li')")
})

observeEvent(input$bf_x_axis, {
