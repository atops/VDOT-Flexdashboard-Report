...
plot_height_css <- "calc(93vh - 200px)"
plot_height_css2 <- "calc(93vh - 165px)"
```
...
plotlyOutput("performance", height = plot_height_css, width = "100%")
...
plotlyOutput("volume", height = plot_height_css2, width = "100%")
...
Equipment {data-orientation=rows}
=====================================
...
plotlyOutput("equipment", plot_height_css2, width = "100%")
...
Watchdog {data-orientation=rows}
=====================================
...
DTOutput("SignalsList")

output$SignalsList <- renderDataTable({
    req(input$zone_group)
    debug(logger, "renderDataTable on filtered_corridors(): filtered_corridors()")
    datatable(filter(isolate(filtered_corridors()), as.integer(as.character(SignalID)) > 0),
        escape = FALSE,
        extensions = "Scroller",
        options = list(
            #deferRender = TRUE,
...
SignalsListDTProxy <- dataTableProxy("SignalsList")

observe({
    DT::replaceData(SignalsListDTProxy, filtered_corridors())
})
```
...
# Move and refactor this once it's working
getMapPalette <- function(data, metric) {
    if (identical(metric, arrivals_on_green)) {
        colorBin(
    ...
    } else if (identical(metric, progression_ratio)) {
        colorBin(
            palette = metric$palette,
            domain = metric$domain,
            bins = 8,
    ...
    } else if (identical(metric, queue_spillback_rate)) {
        colorBin(
            palette = metric$palette,
            domain = metric$domain,
            bins = 5,
    ...
    shinyApp(
        ...
        server = function(input, output, session) {
            ...
            output$SigOpsMap <- renderLeaflet({
                ...
                isolate({
                    pal <- getMapPalette(map_data(), map_metric())
                    # pal <- colorBin(
                    #     palette = "YlGnBu",
...
Signal Details {data-orientation=rows}
=====================================
...
fillCol(
    ...
    plotlyOutput("SignalDetailsPlot"),
    height = "100%",
    flex = c(1, 10)
)
...
output$SignalDetailsPlot <- renderPlotly({
