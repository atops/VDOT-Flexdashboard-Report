...
def get_approach_delay(signalid, date_, det_config, conf, cred, per='H'):
    ...
    atspm = s3io.s3_read_parquet(
        Bucket=bucket,
        Key=posixpath.join(key_prefix, f'atspm/date={date_str}/atspm_{signalid}_{date_str}.parquet'))
    atspm = atspm[atspm.EventCode==132]
    ...
    cd = s3io.s3_read_parquet(
        Bucket=bucket,
        Key=posixpath.join(key_prefix, f'cycles/date={date_str}/cd_{signalid}_{date_str}.parquet'),
    ...
    for date_ in dates:
        try:
            ...
            s3io.s3_write_parquet(
                df,
                Bucket=bucket,
                Key=posixpath.join(key_prefix, f'mark/approach_delay_1hr/date={date_str}/ad_{date_str}.parquet'))
            ...
            s3io.s3_write_parquet(
                df,
                Bucket=bucket,
                Key=posixpath.join(key_prefix, f'mark/approach_delay_15min/date={date_str}/ad_{date_str}.parquet'))
