---
title: "VDOT ATSPM Dashboard (beta)"
output: 
...
#load("Monthly_Report_UI_Functions.Rdata")
...
filtered_corridors <- reactive({
    if (corridor() == "All Corridors") {
        corridors %>%
            filter(signal_group %in% input$signal_group) %>%
...
renderValueBox({
  valueBox(
      value = get_valuebox(mr()$mo$tp, "vph", as_int, 
                           zone = signal_group(), mo = current_month(), break_ = TRUE),
      icon = "fa-bar-chart",
    color = VDOT_BLUE
  )
})
...
renderValueBox({
  valueBox(
    value = get_valuebox(mr()$mo$vpd, "vpd", as_int, 
                           zone = signal_group(), mo = current_month(), break_ = FALSE),
    icon = "fa-area-chart",
    color = VDOT_BLUE
  )
})
...
renderPlotly({
    get_bar_line_dashboard_plot(mr()$wk$tp, mr()$mo$tp, NULL,
                                ...
                                month_ = current_month(), zone_group_ = signal_group(),
